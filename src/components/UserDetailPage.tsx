...
const UserDetailPage = () => {
  ...
  return (
        <div className="max-w-4xl mx-auto p-5">
          ...
          {user && (
            <>
              <div className="bg-white dark:bg-slate-700 dark:text-white shadow-md rounded-lg px-3 py-6 md:px-6 mb-6 ">
              <div className="flex flex-row justify-between">
                ...
                <p className="text-gray-600 pt-4 dark:text-gray-300"><span className="font-medium">Created:</span> {new Date(user.created_at).toLocaleString('fi-FI')}</p>

              </div>
          <div>
            <h2 className=" text-[26px] md:text-[33px] font-bold mb-4 text-center dark:text-white">Posts :</h2>
              ...
              {posts && posts.length > 0 ? (
                posts.map((post) => (
                  <Link key={post.media_id} to='/post' state={post}>
                    <div className="bg-white dark:bg-slate-700 dark:text-white rounded-lg shadow overflow-hidden max-h-55">
                ...
                <div className='flex flex-col gap-6 py-5'>
                  <div className="text-center text-[18px] md:text-[22px] font-semibold w-full dark:text-gray-300">This is user got no Posts</div>
                  <MdOutlinePermMedia className=' text-[88px] md:text-[122px] w-full text-center mb-20 dark:text-gray-300' />
                </div>
              )}
            </div>
          </div>
